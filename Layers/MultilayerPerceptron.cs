...
namespace Bubble
{
    ...
    public class MultilayerPerceptron
    {
        ...
        public struct Network
        {
            ...
            public uint activationFunction;

            public uint derivativeFunction;
            public Vector4[] desiredOutput;
        ...
        ComputeShader _multilayerPerceptron;
        ...
        public MultilayerPerceptron(int[] networkStructure, ActivationFunction activationFunction, float learningRate,
            ComputeShader multilayerPerceptron)
        {
            network = new Network();
            network.layerCount = (uint)networkStructure.Length;
            network.activationFunction = (uint)activationFunction;
            network.derivativeFunction = (uint)activationFunction;
            ...
            for (int i = 0; i < networkStructure.Length; i++)
            {
                ...
                if (i == networkStructure.Length - 1)
                {
                    ...
                    network.desiredOutput = new Vector4[networkStructure[i]];
                }
            ...
            this._multilayerPerceptron = multilayerPerceptron;
        }
        ...
        public float[] Infer(float[] input)
        {
            ...
            _multilayerPerceptron.SetBuffer(0, "network", _networkBuffer);

            int feedForwardKernel = _multilayerPerceptron.FindKernel("feedforward");
            int backPropagateKernel = _multilayerPerceptron.FindKernel("backpopagate");

            _multilayerPerceptron.Dispatch(feedForwardKernel, 4, 4, 1);
        ...
        public float[] Train(float[] input, float[] desiredOutput)
        {
            //Set the Input
            for (int i = 0; i < input.Length / 4; i++)
                network.input[i] = new Vector4(input[i * 4], input[i * 4 + 1], input[i * 4 + 2], input[i * 4 + 3]);
            ...
            for (int i = 0; i < desiredOutput.Length / 4; i++)
                network.desiredOutput[i] = new Vector4(desiredOutput[i * 4], desiredOutput[i * 4 + 1], desiredOutput[i * 4 + 2], desiredOutput[i * 4 + 3]);

            _networkBuffer = new ComputeBuffer(1, sizeof(float) * 4 * _totalSize);
            _multilayerPerceptron.SetBuffer(0, "network", _networkBuffer);

            int feedForwardKernel = _multilayerPerceptron.FindKernel("feedforward");
            int trainingkernel = _multilayerPerceptron.FindKernel("train");

            _multilayerPerceptron.Dispatch(feedForwardKernel, 4, 4, 1);
            _multilayerPerceptron.Dispatch(trainingkernel, 4, 4, 1);
        ...
        public float[] backpropagate()
        {
            ...
            int feedForwardKernel = _multilayerPerceptron.FindKernel("feedforward");
            int backpropagateKernel = _multilayerPerceptron.FindKernel("backpropagate");

            _multilayerPerceptron.Dispatch(feedForwardKernel, 4, 4, 1);
            _multilayerPerceptron.Dispatch(backpropagateKernel, 4, 4, 1);
